				...
				<div class="terminal card w-100">
					<div class="terminal d-flex align-items-center justify-content-between">
						<div class="clickable py-2 px-3 d-flex flex-column">
							<h3 class="d-block">{{ customer.company }}</h3>
							<p>
								<span class="badge badge-success mr-2" v-if="customer.is_active">Activé</span>
						...
						<div class="links d-flex flex-column align-items-stretch bg-light align-self-stretch justify-content-around border-left">
							<a href="" @click.prevent="deactivateClient(customer.id)" v-if="customer.is_active" class="text-danger px-3">Désactiver le client</a>
							<a href="" @click.prevent="activateClient(customer.id)" v-else class="text-danger px-3">Activer le client</a>
							<a href="" @click.prevent="editClient(customer.id)" class="px-3">Modifier le client</a>
						</div>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			deactivateClient: function(id) {
				this.$http.get('customer/'+ id +'/deactivate/')
				.then(resp => {
					if(resp) {
						this.getCustomers();
					}
			...
			activateClient: function(id) {
				this.$http.get('customer/'+ id +'/activate/')
				.then(resp => {
					if(resp) {
						this.getCustomers();
					}
