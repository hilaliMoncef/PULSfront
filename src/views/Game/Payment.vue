<template>
	<div class="col-6 offset-3">
		...
		<div class="row text-center mb-3">
			<img src="@/assets/img/contactless.png" width="100" class="mx-auto" alt="Contact Less" />
		</div>
		<div class="form card py-4 px-3 m-0">
			...
			<div class="row mt-3 text-center">
				<div class="col">
					<button class="btn btn-primary" @click="pay">Payer sans contact</button>
				</div>
			</div>
		</div>
	</div>
		...
		methods: {
			pay: function() {
				if(this.amount > 0) {
					...
					this.payment = {
						'donator': '',
						'terminal': this.$store.state.currentTerminal.id,
						'campaign': this.$store.state.currentCampaign.id,
						'method': 'Contactless',
						'status': 'Accepted',
						'amount': this.amount,
						'currency': 'EUR'
